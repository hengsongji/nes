        ...
        case 7:   /* 0x2007 */
          {
            ...
            if ( !( addr & 0xf ) )  /* 0x3f00 or 0x3f10 */
            {
              ...
              PalTable[ 0x10 ] = PalTable[ 0x14 ] = PalTable[ 0x18 ] = PalTable[ 0x1c ] = (byData&0x3F) | 0x80; //  NesPalette[ byData ] | SPR_FLAG_BIT;
 #else
              PalTable[ 0x00 ] = PalTable[ 0x04 ] = PalTable[ 0x08 ] = PalTable[ 0x0c ] =
              PalTable[ 0x10 ] = PalTable[ 0x14 ] = PalTable[ 0x18 ] = PalTable[ 0x1c ] = NesPalette[ byData ] | SPR_FLAG_BIT;
#endif
            }
            else
	    if ( addr & 3 )
            {
              ...
              PPURAM[ addr ] = byData;
#ifdef NES_WORKFRAME_SLIM
              PalTable[ addr & 0x1f ] = (byData&0x3F); // NesPalette[ byData ];
#else
    ...
    case 0x4000:  /* Sound */
      switch ( wAddr & 0x1f )
      {
        ...
        case 0x14:  /* 0x4014 */
          ...
          {
            case 0x0:  /* RAM */
              InfoNES_MemoryCopy( SPRRAM, &RAM[ ( (WORD)byData << 8 ) & 0x7ff ], NES_SPRRAM_SIZE );
              break;

            case 0x3:  /* SRAM */
              InfoNES_MemoryCopy( SPRRAM, &SRAM[ ( (WORD)byData << 8 ) & 0x1fff ], NES_SPRRAM_SIZE );
              break;

            case 0x4:  /* ROM BANK 0 */
              InfoNES_MemoryCopy( SPRRAM, &ROMBANK0[ ( (WORD)byData << 8 ) & 0x1fff ], NES_SPRRAM_SIZE );
              break;

            case 0x5:  /* ROM BANK 1 */
              InfoNES_MemoryCopy( SPRRAM, &ROMBANK1[ ( (WORD)byData << 8 ) & 0x1fff ], NES_SPRRAM_SIZE );
              break;

            case 0x6:  /* ROM BANK 2 */
              InfoNES_MemoryCopy( SPRRAM, &ROMBANK2[ ( (WORD)byData << 8 ) & 0x1fff ], NES_SPRRAM_SIZE );
              break;

            case 0x7:  /* ROM BANK 3 */
              InfoNES_MemoryCopy( SPRRAM, &ROMBANK3[ ( (WORD)byData << 8 ) & 0x1fff ], NES_SPRRAM_SIZE );
              break;
          }
          break;
